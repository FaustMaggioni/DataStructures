...
private static int TAMAÑO = 20;
    ...
    public boolean insertar(Comparable e, ClaveCliente clave){
       
        boolean exito=this.ultimo<this.TAMAÑO-1;
        if(exito){
            this.heap[ultimo+1]= new NodoPromocion(clave, (int)e);
            this.ultimo++; 
            if(ultimo!=1){
                int posActual=ultimo;
                while(posActual>1 && ((this.heap[posActual].getCant()).compareTo((this.heap[(int)posActual/2]).getCant())>0)){
    ...
    private void hacerBajar(int posPadre) {
        ...
        while (!salir) {
            ...
            if (posH <= this.ultimo) {
                if (posH < this.ultimo) {
                    if (this.heap[posH + 1].getCant().compareTo(this.heap[posH].getCant()) > 0) {
                        posH++;
                    }
                }

                if (this.heap[posH].getCant().compareTo(temp.getCant()) > 0) {
