...
public class Grafo {
    ...
    public Lista caminoMasCorto(Object origen, Object destino) {
        ...
        if (verticeOr != null) {
            ...
            masLargo = caminoMasCortoAux(verticeOr, destino, visitados, masLargo, masLargoActual);
        }
        return masLargo;
    }

    private Lista caminoMasCortoAux(NodoVert n, Object destino, Lista vis, Lista masLargo, Lista masLargoActual) {
        if (n != null) {
            ...
            if (n.getElem().equals(destino)) {
                ...
                while (ady != null) {
                    if (vis.localizar(ady.getvertice().getElem()) < 0) {
                        masLargo = caminoMasCortoAux(ady.getvertice(), destino, vis, masLargo, masLargoActual);
                    }
                    ady = ady.getSigAdy();
                }
            }
    ...
    public Lista liviano(Object origen, Object destino) {
        ...
        if (verticeOr != null) {
            ...
            Lista masLargoActual = new Lista();
            menorPeso = new Lista();
            menorPeso = livianoAux(verticeOr, destino, new NodoSuma(0), new NodoSuma(0), menorPeso, masLargoActual, visitados);
            
        }
        return menorPeso;
    }

    private Lista livianoAux(NodoVert n, Object destino, NodoSuma sumaMenor, NodoSuma sumaActual, Lista menor, Lista actual, Lista vis) {
        if (n != null) {
            vis.insertar(n.getElem(), vis.longitud() + 1);
            ...
            if (n.getElem().equals(destino)) {
                ...
                while (aux != null) {
                    if (vis.localizar(aux.getvertice().getElem()) == -1) {
                        sumaActual.setValor(sumaActual.getValor()+(int) aux.getEtiqueta() ); 
                        menor = livianoAux(aux.getvertice(), destino, sumaMenor, sumaActual, menor, actual, vis);
                        sumaActual.setValor(sumaActual.getValor()-(int) aux.getEtiqueta() );
                    }
                    aux = aux.getSigAdy();
                }
    ...
    public boolean eliminarArco(Object e, Object e2) {
        boolean res = false;
        NodoVert aux = this.inicio;
        ...
        if (origen != null && destino != null) {
            ...
            if (a.getvertice().getElem().equals(e2)) {
                eliminarB(destino, e);
                origen.setPrimerAdy(a.getSigAdy());
            } else {
                while (a.getSigAdy() != null) {
                    if (a.getSigAdy().getvertice().getElem().equals(e2)) {
                        eliminarB(destino, e);
    ...
    private void eliminarB(NodoVert destino, Object e) {
        NodoAdy b = destino.getPrimerAdy();
        if (b.getvertice().getElem().equals(e)) {
            destino.setPrimerAdy(b.getSigAdy());
        } else {
            while (b.getSigAdy() != null) {
                if (b.getSigAdy().getvertice().getElem().equals(e)) {
